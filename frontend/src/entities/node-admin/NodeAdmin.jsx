...
const NodeAdmin = ({adminId}) => {
  const { adminFirstname, adminLastname, adminEmail } = useSelector(state => state.groupsNodes.admins[adminId]);

  return (
    <div className="main__node-admin">
        <div className="main__node-admin-comp">
            <img className="main__node-admin-icon" src={adminIcon} alt="admin icon from flaticon" />
        ...
        <div className="main__node-admin-email-comp" onClick={() => {location.href=`mailto:${adminEmail}`}}>
            <img className="main__node-admin-email-icon" src={emailIcon} alt="email icon from flaticon" />
