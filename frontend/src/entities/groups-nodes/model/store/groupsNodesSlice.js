    ...
    builder.addCase(fetchGroupsNodes.fulfilled, (state, action) => {
      ...
      const data = action.payload.reduce((acc, item) => {
        acc.groups[item.group_id] = {
          ...
          nodeId: [...(acc.groups[item.group_id]?.nodeId || []), item.node_id],
        };

        acc.nodes[item.node_id] = {
          nodeName: item.node_name,
          statusId: item.node_status_id,
          interfaceId: item.interface_id,
          adminId: item.admin_id,
          appId: item.app_id,
          groupId: [...(acc.nodes[item.node_id]?.groupId || []), item.group_id],
        };

        acc.interfaces[item.interface_id] = {
          interfaceName: item.interface_name,
          interfaceStatusId: item.interface_status_id,
        };

        acc.apps[item.app_id] = { appName: item.app_name };

        acc.statuses[item.node_status_id] = {
          statusName: item.node_status_name,
          statusColor: item.node_status_color,
        };

        if (!acc.statuses[item.interface_status_id]) {
          acc.statuses[item.interface_status_id] = {
            statusName: item.interface_status_name,
            statusColor: item.interface_status_color,
          };
        }

        acc.admins[item.admin_id] = {
          adminFirstname: item.admin_firstname,
          adminLastname: item.admin_lastname,
