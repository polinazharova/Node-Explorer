...
const GroupsBlock = () => {
  const { groups, nodes, status, error } = useSelector(
    (state) => state.groupsNodes,
  );
  ...
  const groupsLen = useMemo(() => Object.keys(groups).length, [groups]);

  const nodesLen = useMemo(() => Object.keys(nodes).length, [nodes]);
  ...
  if (error && status !== "succeeded") {
    return <div className="main__groups-block">{error}</div>;
  }

  if (status === "loading" || status === "idle") {
    return <div className="main__groups-block">Загрузка...</div>;
  }
  ...
  return (
    <div className="main__groups-block">
      ...
      <Divider />
      <span className="main__groups-block-info">
        Кол-во групп: {groupsLen}, кол-во нод: {nodesLen}
      </span>
      <span className="main__groups-block-info">
        Кол-во нод в выбранной группе: {selectedGroupNodes.length}
      </span>
      <GroupsList />
      <Divider />
