...
const NodesSwitcher = () => {
    ...
    const handleNextClick = () => {
        dispatch(setIndex(firstShownIndex + 5 >= selectedGroupNodes.length ? firstShownIndex : firstShownIndex + 5));
    }
  ...
  return (
    <div className="main__nodes-switcher">
        <span className={`main__nodes-switcher-prev ${!firstShownIndex ? "main__nodes-switcher_disabled" : ""}`} onClick={handlePrevClick}>Пред.</span>
        <span className={`main__nodes-switcher-next ${firstShownIndex + 5 >= selectedGroupNodes.length ? "main__nodes-switcher_disabled" : ""}`} onClick={handleNextClick}>След.</span>
    </div>
  )
}
